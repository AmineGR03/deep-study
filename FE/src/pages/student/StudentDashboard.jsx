...
function SpecialiteOrFiliere({ user }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        if (user.specialite_id && user.annee_id) {
          // 4ème/5ème année avec spécialité → "4 DDSI"
          const [specRes, anneeRes] = await Promise.all([
            axiosInstance.get(`/data/specialite/${user.specialite_id}`),
            axiosInstance.get('/data/annees'),
          ])
          const annee = anneeRes.data.find(a => a._id === user.annee_id)
          setLabel(`${annee?.niveau ?? ''} ${specRes.data.nom}`)

        } else if (user.filiere_id && user.annee_id) {
          // Sans spécialité → "3 IIR"
          const [filiereRes, anneeRes] = await Promise.all([
            axiosInstance.get('/data/filieres'),
            axiosInstance.get('/data/annees'),
          ])
          const filiere = filiereRes.data.find(f => f._id === user.filiere_id)
          const annee   = anneeRes.data.find(a => a._id === user.annee_id)
          setLabel(`${annee?.niveau ?? ''} ${filiere?.nom ?? '—'}`)

        } else if (user.filiere_id) {
          // Fallback sans année
          const res = await axiosInstance.get('/data/filieres')
          const filiere = res.data.find(f => f._id === user.filiere_id)
          setLabel(filiere?.nom || '—')
        }
      } catch {
        setLabel('—')
      }
  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        ...
        // Documents récents de la filière
        if (user?.filiere_id) {
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className="mb-8">
        ...
        <p className="ds-muted">Bienvenue sur DeepStudy — votre assistant pédagogique IA</p>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            ...
            <div className="font-display text-lg font-bold text-orange-400 mb-1">
                <SpecialiteOrFiliere user={user} />
            </div>
            <p className="ds-muted text-sm">
                {user?.specialite_id ? 'Votre spécialité' : 'Votre filière'}
            </p>
            </div>
